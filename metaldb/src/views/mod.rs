...
///
/// ```
/// use metaldb::{access::{Access, RawAccessMut}, ListIndex, MapIndex};
///
...
impl<T: RawAccess> ViewInner<T> {
    ...
    where
        I: iter::Iterator,
        <I as iter::Iterator>::Item: AsRef<[u8]>,
    {
        ...
        let (mut res, db_keys) = keys.into_iter().enumerate().fold(
            ...
            |(mut res, mut db_keys), (idx, key)| {
                if let Some(Ok(item)) = changes.map(|changes| changes.get(key.as_ref())) {
                    res.push(item);
                } else {
                    res.push(None);
